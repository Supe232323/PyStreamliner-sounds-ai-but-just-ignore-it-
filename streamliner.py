...
parallel processing via --jobs, config files, and path excludes.
"""
...
# ─── SourceAnalyzer (unchanged core logic) ────────────────────────────────────
class SourceAnalyzer:
    ...
    def _find_unused_variables(self) -> list[Warning]:
        ...
        for name, lineno in self._collect_assigned_names():
            ...
            if name in self._all_names or name in self._used_names:
                continue
            warnings.append(Warning("unused_variable", name, lineno, f"⚠ Unused variable '{name}' at line {lineno}"))
        return warnings
    ...
    def _find_vague_names(self) -> list[Warning]:
        ...
        for name, lineno in self._collect_assigned_names():
            ...
            if name.lower() in VAGUE_NAMES:
                warnings.append(Warning("vague_name", name, lineno, f"⚠ Vague variable name '{name}' at line {lineno}"))
        return warnings
    ...
    def _find_shadowed_builtins(self) -> list[Warning]:
        ...
        for name, lineno in self._collect_assigned_names():
            ...
            seen.add((name, lineno))
            if name in SHADOWED:
                warnings.append(Warning("shadowed_builtin", name, lineno, f"⚠ Variable '{name}' shadows a built-in at line {lineno}"))
        return warnings
    ...
    def _find_dangerous_calls(self) -> list[Warning]:
        ...
        for node in ast.walk(self._tree):
            ...
            if isinstance(node.func, ast.Name) and node.func.id in DANGEROUS_NAMES:
                warnings.append(Warning("dangerous_call", node.func.id, node.lineno, f"⚠ Dangerous call '{node.func.id}()' at line {node.lineno}"))
                continue
            if isinstance(node.func, ast.Attribute):
                ...
                if isinstance(node.func.value, ast.Name):
                    ...
                    if mod in {"pickle", "marshal", "shelve"} and attr in PICKLE_FUNCS:
                        warnings.append(Warning("dangerous_call", f"{mod}.{attr}", node.lineno, f"⚠ Dangerous call '{mod}.{attr}()' at line {node.lineno}"))
                        continue
                    if mod == "os" and attr in OS_DANGEROUS:
                        warnings.append(Warning("dangerous_call", f"os.{attr}", node.lineno, f"⚠ Dangerous call 'os.{attr}()' at line {node.lineno}"))
                        continue
                    if mod == "yaml" and attr == "load":
                        has_safe = any(
                            kw.arg in {"Loader", "loader"} and isinstance(kw.value, ast.Attribute) and kw.value.attr in {"SafeLoader", "CSafeLoader"}
                            for kw in node.keywords
                        )
                        if not has_safe:
                            warnings.append(Warning("dangerous_call", "yaml.load", node.lineno, f"⚠ Dangerous call 'yaml.load()' without SafeLoader at line {node.lineno}"))
                        continue
                if isinstance(node.func.value, ast.Name) and node.func.value.id == "subprocess":
                    for kw in node.keywords:
                        if kw.arg == "shell" and isinstance(kw.value, ast.Constant) and kw.value.value is True:
                            warnings.append(Warning("dangerous_call", f"subprocess.{attr}", node.lineno, f"⚠ subprocess.{attr}() called with shell=True at line {node.lineno}"))
                            break
        return warnings
            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name) and isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
                        ...
                        if name_lower in SECRET_NAMES or any(s in name_lower for s in ("password", "secret", "token", "api_key")):
                            ...
                            if key not in seen:
                                seen.add(key)
                                warnings.append(Warning("hardcoded_secret", target.id, target.lineno, f"⚠ Possible hardcoded secret in '{target.id}' at line {target.lineno}"))
            elif isinstance(node, ast.AnnAssign) and node.value is not None:
                if isinstance(node.target, ast.Name) and isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
                    ...
                    if name_lower in SECRET_NAMES or any(s in name_lower for s in ("password", "secret", "token", "api_key")):
                        ...
                        if key not in seen:
                            seen.add(key)
                            warnings.append(Warning("hardcoded_secret", node.target.id, node.target.lineno, f"⚠ Possible hardcoded secret in '{node.target.id}' at line {node.target.lineno}"))
        return warnings

    def _find_assert_used(self) -> list[Warning]:
        return [
            Warning("assert_used", "assert", node.lineno, f"⚠ assert used at line {node.lineno} (stripped with -O; do not use for security checks)")
            for node in ast.walk(self._tree) if isinstance(node, ast.Assert)
        ]
    ...
    def _find_broad_excepts(self) -> list[Warning]:
        ...
        for node in ast.walk(self._tree):
            ...
            if node.type is None:
                warnings.append(Warning("broad_except", "except:", node.lineno, f"⚠ Bare 'except:' at line {node.lineno}"))
            elif isinstance(node.type, ast.Name) and node.type.id == "Exception":
                warnings.append(Warning("broad_except", "except Exception", node.lineno, f"⚠ Broad 'except Exception' at line {node.lineno}"))
        return warnings
...
def main() -> int:
    ...
    # Merge exclude paths from config + CLI
    ...
    if args.stdin:
        # stdin path left simple (no config complexity needed)
    ...
    args_dict = {
        ...
        "json": args.json, "summary_threshold": args.summary_threshold,
    }
    ...
    if args.json:
        ...
        if len(results) >= args.summary_threshold:
